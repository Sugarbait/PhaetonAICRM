...
async function migrateUsers() {
  ...
  try {
    // Fetch ARTLEE users from old database
    ...
    // Insert users into new database (batch insert)
...
async function migrateUserSettings() {
  ...
  try {
    // Fetch ARTLEE user settings from old database
    ...
    // Insert user settings into new database
    const { data: inserted, error: insertError } = await newDb
      .from('user_settings')
      .insert(settings)
      .select()
...
async function migrateUserProfiles() {
  ...
  try {
    // Fetch ARTLEE user profiles from old database
    ...
    // Insert user profiles into new database
    const { data: inserted, error: insertError } = await newDb
      .from('user_profiles')
      .insert(profiles)
      .select()
...
async function migrateAuditLogs() {
  ...
  try {
    // Fetch ARTLEE audit logs from old database
    // Note: Audit logs can be large, so we'll paginate
    let allLogs = []
    let page = 0
    const pageSize = 1000
    let hasMore = true
    ...
    // Insert audit logs in batches (Supabase has 1000 row limit per insert)
...
async function migrateNotes() {
  ...
  try {
    // Fetch ARTLEE notes from old database
    ...
    // Insert notes into new database
    const { data: inserted, error: insertError } = await newDb
      .from('notes')
      .insert(notes)
      .select()
...
async function migrateSystemCredentials() {
  ...
  try {
    // Fetch ARTLEE system credentials from old database
    ...
    // Insert system credentials into new database
    const { data: inserted, error: insertError } = await newDb
      .from('system_credentials')
      .insert(credentials)
      .select()
...
async function migrateCompanySettings() {
  ...
  try {
    // Fetch ARTLEE company settings from old database
    const { data: settings, error: fetchError } = await oldDb
    ...
    // Insert company settings into new database
    const { data: inserted, error: insertError } = await newDb
      .from('company_settings')
      .insert(settings)
      .select()
