-- Phaeton AI CRM Database Schema Setup
...
-- ============================================
-- Link the Supabase Auth user to the users table
...
CREATE POLICY IF NOT EXISTS "Users are isolated by tenant"
    ON public.users
    FOR ALL
    USING (tenant_id = 'phaeton_ai');

-- Audit logs policies
CREATE POLICY IF NOT EXISTS "Audit logs are isolated by tenant"
    ON public.audit_logs
    FOR ALL
    USING (tenant_id = 'phaeton_ai');

-- User settings policies
CREATE POLICY IF NOT EXISTS "User settings are isolated by tenant"
    ON public.user_settings
    FOR ALL
    USING (tenant_id = 'phaeton_ai');

-- Notes policies
CREATE POLICY IF NOT EXISTS "Notes are isolated by tenant"
    ON public.notes
    FOR ALL
    USING (tenant_id = 'phaeton_ai');

-- Company settings policies
CREATE POLICY IF NOT EXISTS "Company settings are isolated by tenant"
...
AND table_name IN ('users', 'user_credentials', 'audit_logs', 'user_settings')
