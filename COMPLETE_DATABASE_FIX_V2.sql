-- ============================================================================
-- ARTLEE CRM - COMPLETE DATABASE FIX V2 (ALL-IN-ONE)
-- ============================================================================
-- Fixed version that adds tenant_id to ALL tables before creating indexes
-- ============================================================================

-- ============================================================================
-- PART 1: ADD tenant_id TO ALL EXISTING TABLES FIRST
-- ============================================================================

-- Add tenant_id to users if missing
ALTER TABLE public.users ADD COLUMN IF NOT EXISTS tenant_id TEXT DEFAULT 'artlee' NOT NULL;

-- Add tenant_id to user_settings if missing
ALTER TABLE public.user_settings ADD COLUMN IF NOT EXISTS tenant_id TEXT DEFAULT 'artlee' NOT NULL;

-- Add tenant_id to audit_logs if missing
ALTER TABLE public.audit_logs ADD COLUMN IF NOT EXISTS tenant_id TEXT DEFAULT 'artlee' NOT NULL;

-- Add tenant_id to failed_login_attempts if missing
ALTER TABLE public.failed_login_attempts ADD COLUMN IF NOT EXISTS tenant_id TEXT DEFAULT 'artlee' NOT NULL;

-- Add tenant_id to user_credentials if missing (THIS WAS MISSING)
ALTER TABLE public.user_credentials ADD COLUMN IF NOT EXISTS tenant_id TEXT DEFAULT 'artlee' NOT NULL;
...
-- ============================================================================
-- PART 5: CREATE user_profiles TABLE IF MISSING
-- ============================================================================
...
-- ============================================================================
-- PART 6: DISABLE RLS ON ALL TABLES TEMPORARILY
-- ============================================================================
...
-- ============================================================================
-- PART 8: GRANT FULL PERMISSIONS TO ALL ROLES
-- ============================================================================
...
-- ============================================================================
-- PART 9: ENABLE RLS WITH PERMISSIVE POLICIES
-- ============================================================================
...
-- ============================================================================
-- PART 10: ADD INDEXES FOR PERFORMANCE (AFTER tenant_id columns exist)
-- ============================================================================
...
-- ============================================================================
-- PART 11: INSERT INITIAL COMPANY SETTINGS
-- ============================================================================
...
-- ============================================================================
-- PART 12: SET PROPER COLUMN DEFAULTS
-- ============================================================================
...
-- Ensure user_settings has proper defaults
ALTER TABLE user_settings
  ALTER COLUMN tenant_id SET DEFAULT 'artlee';

-- Ensure user_credentials has proper defaults
...
-- Test user_credentials
DO $$
DECLARE
  test_count INTEGER;
BEGIN
  SELECT COUNT(*) INTO test_count FROM user_credentials;
  RAISE NOTICE '✅ user_credentials table accessible: % rows', test_count;
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE '❌ ERROR accessing user_credentials: %', SQLERRM;
END $$;
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '========================================';
  RAISE NOTICE '✅ COMPLETE DATABASE FIX V2 FINISHED';
  RAISE NOTICE '========================================';
  RAISE NOTICE '';
  RAISE NOTICE 'All tables created and configured:';
  RAISE NOTICE '  ✓ tenant_id added to ALL tables';
  ...
  RAISE NOTICE '  ✓ user_credentials - tenant_id column added';
  ...
  RAISE NOTICE '  1. Open: I:\Apps Back Up\ARTLEE CRM\clear-localStorage.html';
  RAISE NOTICE '  2. Click: Clear ALL localStorage button';
  RAISE NOTICE '  3. Hard refresh browser (Ctrl+F5)';
  RAISE NOTICE '  4. Go to http://localhost:3001';
  RAISE NOTICE '  5. Register first user (becomes Super User)';
  RAISE NOTICE '';
  RAISE NOTICE '========================================';
END $$;

-- ============================================================================
-- END OF COMPLETE DATABASE FIX V2
-- ============================================================================
